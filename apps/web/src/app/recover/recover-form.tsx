...
export function RecoverForm() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    setPending(true);
    try {
      const res = await fetch("/api/auth/sign-in/recovery", {
        method: "POST",
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-4">
      ...
      <label className="flex flex-col gap-1.5">
        <span className="text-sm font-medium text-fg">Recovery code</span>
        <input
          type="text"
          autoComplete="off"
          spellCheck={false}
          ...
          className="rounded-md border border-border bg-surface px-3 py-2 font-mono text-base text-fg placeholder:text-subtle focus:border-border-strong"
        />
      </label>
      {error ? (
